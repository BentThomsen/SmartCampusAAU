...
@interface UITextStepperField ()
- (void)      initializeControl;
- (NSString*) getPlaceholderText;
- (void)      didChangeTextField;
@end

@implementation UITextStepperField
...
{
    if (self.delegate) {
        self.textField.text = [self.delegate textStepperField:self textForValue:pflValue];
    } else {
    ...
    if (pflValue!=_current) {
        [self.delegate textStepperField:self valueDidChange:pflValue];
        _current = pflValue;
    }
}
